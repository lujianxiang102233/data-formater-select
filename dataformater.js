...
TimeFormater.fn = TimeFormater.prototype = {
  ...
  MM: function () {
    var that = this
    that.month =
      new Date(that.timeNumber).getMonth() + 1 < 10
        ? '0' + (new Date(that.timeNumber).getMonth() + 1)
        : new Date(that.timeNumber).getMonth() + 1
    return that.month
  },
  DD: function () {
    var that = this
    that.day =
      new Date(that.timeNumber).getDate() < 10
        ? '0' + new Date(that.timeNumber).getDate()
        : new Date(that.timeNumber).getDate()
    return that.day
  },
  hh: function () {
    var that = this
    that.hours =
      new Date(that.timeNumber).getHours() < 10
        ? '0' + new Date(that.timeNumber).getHours()
        : new Date(that.timeNumber).getHours()
    return that.hours
  },
  mm: function () {
    var that = this
    that.minutes =
      new Date(that.timeNumber).getMinutes() < 10
        ? '0' + new Date(that.timeNumber).getMinutes()
        : new Date(that.timeNumber).getMinutes()
    return that.minutes
  },
  ss: function () {
    var that = this
    that.seconds =
      new Date(that.timeNumber).getSeconds() < 10
        ? '0' + new Date(that.timeNumber).getSeconds()
        : new Date(that.timeNumber).getSeconds()
    return that.seconds
  },
}
